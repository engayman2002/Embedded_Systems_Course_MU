...
# 1 "./lib/keypad/keypad.h" 1
# 12 "./lib/keypad/keypad.h"
...
# 1 "./lib/timer/timer.h" 1
# 30 "./lib/timer/timer.h"
...
int Current_Year ;
char month,Current_Day,Current_Hour,Current_Minute;

unsigned long long Get_Real_Number(char num,char looping)
{
    char im=48;
...
void Required_Delay(unsigned long Required_Time)
{
    unsigned long stime=0,etime=0;
    etime = timer_get_time_since_init();
    while(etime-stime<=Required_Time)
    {
        etime = timer_get_time_since_init();
    }
...
void main(void) {
    ...
    lcd_cmd((0x01));
    Current_Year =1970;
    month=1,Current_Day=1,Current_Hour=0,Current_Minute=0;
    lcd_out(1,1,"Welcome name");
    Required_Delay(2000);
    lcd_cmd((0x01));
    char My_Buffer[11];
    lcd_out(1,1,"Time_Max_10Digit");
    lcd_out(2,1,"Type =");
    char end=0;
    {
        char *c=My_Buffer;
lcd_set_cursor(2,7);
    ...
    while(*c!=0)
    {
        Seconds+= Get_Real_Number(*c,end);
        end--;
        c++;
    }
    }
    while( (( (Current_Year%4==0) && ( (Current_Year%100!=0) || (Current_Year%400==0) ) ) && Seconds>=31622400) || (Seconds>=31536000 && !( (Current_Year%4==0) && ( (Current_Year%100!=0) || (Current_Year%400==0) ) )))
    {
    ...
    if(Seconds>=86400)
    {
        int num_of_Current_Days=Seconds/(86400);
        int desired = 0 ;
        int i=0;
        for( i=0; i<11 && desired + months[i] <num_of_Current_Days && num_of_Current_Days!=0 ;i++)
        {
            if(i==1 && ( (Current_Year%4==0) && ( (Current_Year%100!=0) || (Current_Year%400==0) ) ) )
            {
                desired+=(months[i]+1);
                continue;
            }
        ...
        month = i+1;
        Seconds-=(desired*86400);
    }
    if(Seconds>=86400)
    {
        Current_Day = Seconds / 86400;
        Current_Day++;
    ...
    if(Seconds>=60)
    {
        ...
        Seconds -= (Current_Minute*60);
    }
    while(1)
    {
    lcd_set_cursor(1,1);
    Print(Current_Year,4);
    lcd_chr_cp('-');
    Print(month,2);
    lcd_chr_cp('-');
    Print(Current_Day,2);
    lcd_set_cursor(2,1);
    Print(Current_Hour,2);
    lcd_chr_cp(':');
    Print(Current_Minute,2);
    lcd_chr_cp(':');
    Print((char)Seconds,2);
     Required_Delay(1000);
        if(Seconds==59)
        {
            Seconds=0;
            if(Current_Minute==59)
            {
                ...
                if(Current_Hour==23)
                {
                    ...
                    if(( (Current_Year%4==0) && ( (Current_Year%100!=0) || (Current_Year%400==0) ) ) && month == 2 && Current_Day==29)
                    {
                        Current_Day = 0;
                        month++;
                    }
                    else if(( (Current_Year%4==0) && ( (Current_Year%100!=0) || (Current_Year%400==0) ) ) && month == 2 && Current_Day ==28)
                    {
                        Current_Day=29;
                    }
                    else if(( (Current_Year%4==0) && ( (Current_Year%100!=0) || (Current_Year%400==0) ) ) && month == 2)
                    {
                        Current_Day++;
                    }
                    else
                    {
                        if(Current_Day < months[month-1] )
                        {
                            Current_Day++;
                        }
                        else
